 ...
 */
public class NewTutorPanel extends SubmitFormPanel
{
	 ...
	 */
	public NewTutorPanel( MainWindow parent )
	{
		super(parent);
		...
		JLabel labelTitle = new JLabel( "Add New Tutor", SwingConstants.LEADING  );
		...
		JLabel labelHours = new JLabel( "Available Hours", SwingConstants.TRAILING );
		...
		panelDetails.add( labelHours );
		panelDetails.add( textHours );
	...
	public void submit()
	{
		...
		inputHours = textHours.getText();


		// Check for Incomplete information.
		if ( inputFirstName == null || inputFirstName.length() < 2 )
		{
			JOptionPane.showMessageDialog( windowParent, "No First Name", "Error", JOptionPane.ERROR_MESSAGE );
		}
		else if ( inputFamilyName == null || inputFamilyName.length() < 2  )
		{
			JOptionPane.showMessageDialog( windowParent, "No Family Name", "Error", JOptionPane.ERROR_MESSAGE );
		}
		else if ( inputContact == null || inputContact.length() < 2 )
		{
			JOptionPane.showMessageDialog( windowParent, "No Contact Details", "Error", JOptionPane.ERROR_MESSAGE );
		}
		else if ( inputHours == null || inputHours.length() < 1 )
		{
			JOptionPane.showMessageDialog( windowParent, "No Hours set", "Error", JOptionPane.ERROR_MESSAGE );
		}
		else
		{
			...
			/* Add new Student */
			windowParent.getCommandClass().tutorAdd(inputFirstName, inputFamilyName, inputContact, inputHours, inputStudentSubject );
		}
